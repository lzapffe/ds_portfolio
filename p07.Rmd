...
I tried attaching the html output to this document, which worked fine when rendering, but the webpage template didn't seem to do well with it. I also tried re-building the webpage in Rstudio to refresh everything and convert the pages to jpg, but nothing really worked that well. So, I will instead link to the html file in my repository and the easiest way to see it is probably to download the html file and view it in a browser.

<br>

**You can find the output file for this project [here](https://github.com/lzapffe/ds_portfolio/blob/main/Data_Wrangling_rmd.html)**