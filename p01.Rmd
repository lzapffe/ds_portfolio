...
- [ ] creates a unchecked box, while [x] creates a checked box. In html, the formatting gets weird with checked boxes. It turns bold automatically and adds extra space for the next line. However, if making it a Github document, it shows up with the expected formatting
...
Note, if you write the code for in-text r code (tick mark, r, code, and tick mark) in this comment, it will actually run that code

You get **bold** text from adding two stars before and after the text you want in bold
...
This is a copy of the improved contract. There might be minor variations between this version and the final version attached in the ds_portfolio repository. The code should be more or less the same between the two versions.

All the text and comments here are the same as in the contract templates. The code comments have been changed out with block quotes, as the hash tag symbol would have been rendered as a header if not inside a code block.
...
1)  Earn “Satisfactory” on n_labs self-paced lab assignments on the
...
2)  Produce at least n_portfolios “Satisfactory” portfolio pieces on
...
- [ ] **string_details(monday_week_one, 10, 10, modules_list)**
